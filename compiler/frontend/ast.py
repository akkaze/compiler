...
from compiler.frontend import OutputIrrelevantMaker
from compiler.frontend import SymbolResolver
from compiler.frontend import TypeChecker
class AST(object):
    scope = None
    definition_nodes = []
    class_entities = []
    ...
    def eliminate_output_irrelevant_node(self):
        if len(self.class_entities) != 0:
            return
        analyzer = OutputIrrelevantMaker(self)
        analyzer.visit_definitions(self.definition_nodes)
    def check_type(self):

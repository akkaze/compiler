    ...
    def visit(self, node):
        if isinstance(node, FunctionDefNode):
            ...
            if init:
                ...
                self.check_compatibility(node.location, \
            ...
            if node.body:
                ...
                self.loop_depth -= 1
            self.check_compatibility(node.location \
                nodecond.type, self.bool_type, True)
            return
            ...
            if op == BinaryOpNode.BinaryOp.MUL or \
                ...
                self.check_compatibility(node.left.location, \
                    ltype, self.integer_type, True)
                self.check_compatibility(node.right.location, \
            ...
            if self.current_function:
                self.current_function.add_call(self.entity)
            ...
            for i in range(base, len(params)):
                ...
                self.check_compatibility(expr.location, \
            ...
            if base != 0:
                if isinstance(node.expr, MemberNode):
                    ...
                    node.add_thispointer(self.current_function.params[0])
            return
            ...
            self.check_compatibility(node.index.location, node.index.type, \

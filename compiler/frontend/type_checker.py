        ...
        if not real.is_compatible(expect):
            ...
            if is_expected:
                message += 'Invalid type ' + \
                    str(real) + ', expecting ' + str(expect)
            else:
                message += 'Incompatible type ' + \
                    str(real) + ', and ' + str(expect)
            raise SemanticError(loc, message)

    def visit(self, node):
        if isinstance(node, FunctionDefNode):
            self.current_function = node.entity
            if not self.current_function.is_constructor and not self.current_function.return_type:
                raise SemanticError(node.location, 'expecting a return type')
            ...
            if init:
                ...
                TypeChecker.check_compatibility(
                    node.location, init.type, node.entity.type, False)
            if node.entity.type.is_void:
                raise SemanticError(
                    node.location, 'Cannot set void type for variable')
            return
            ...
            TypeChecker.check_compatibility(
                node.location, node.cond.type, self.bool_type, True)
            return
            ...
            if self.current_function.is_constructor:
                if node.expr:
                    raise SemanticError(
                        node.location, 'cannot return in constructor')
            else:
                if node.expr:
                    self.visit_expr(node.expr)
                    TypeChecker.check_compatibility(
                        node.location, node.expr.type, self.current_function.return_type, True)
                else:
                    if not self.current_function.return_type.is_void:
                        raise SemanticError(
                            node.location, 'cannot return to void')
            return
            ...
            if not node.lhs.is_assignable:
                raise SemanticError(
                    node.location, 'LHS of \'=\' is not assignable')
            ...
            if isinstance(node.expr, MemberNode) or \
                    (isinstance(node.expr, VariableNode) and node.expr.is_member):
                base = 1
            if len(params) - base != len(exprs):
                raise SemanticError(node.location,
                                    'Incompatible parameter number in function ' + node.callee_name + ' : ' +
                                    str(len(exprs)) + ', expecting ' + str(len(params) - base))
